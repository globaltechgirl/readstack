...
const Audiobooks: FC = () => {
  ...
  useEffect(() => {
    const loadAudiobooksBooks = async () => {
      ...
      try {
        ...
        const formatted: BookDisplay[] = combinedBooks
          ...
          .filter((book) => {
            ...
            return TOP_AUDIOBOOKS_GENRES.some(
        ...
        <Box style={styles.audiobooksWrapper}>
          {loading && !initialLoadDone ? (
            <Center style={{ width: "100%", padding: 50 }}>
              <Text style={styles.centerText}>Loading audiobooks books...</Text>
            </Center>
          ) : allBooks.length === 0 ? (
            <Center style={{ width: "100%", padding: 50 }}>
              <Text style={styles.centerText}>No audiobooks books found.</Text>
            </Center>
          ) : (
