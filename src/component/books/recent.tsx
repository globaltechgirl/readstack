  ...
  useEffect(() => {
    const loadCurrentlyReadingBooks = async () => {
      ...
      try {
        ...
        if (!shelvesResponse?.currentlyReading || shelvesResponse.currentlyReading.length === 0) {
          setAllBooks([]);
          setInitialLoadDone(true);
          return;
        }

        const formatted: BookDisplay[] = shelvesResponse.currentlyReading.map((item: ApiBook) => ({
        ...
        <Box style={styles.recentWrapper}>
          {loading && !initialLoadDone ? (
            <Center style={{ width: "100%", padding: 50 }}>
              <Text style={styles.centerText}>Loading recent books...</Text>
            </Center>
          ) : allBooks.length === 0 ? (
            <Center style={{ width: "100%", padding: 50 }}>
              <Text style={styles.centerText}>No recent books found.</Text>
            </Center>
          ) : (
                ...
                >
                  <Box style={styles.bookWrapper}>
                    ...
                    <Box
                      ...
                      onClick={() => navigate(`/category/${book.isbn}`, { state: { book } })}
                    >
                      <Box
                        onClick={(e) => {
                          e.stopPropagation();
