  ...
  useEffect(() => {
    const loadReadBooks = async () => {
      ...
      try {
        ...
        if (!shelvesResponse?.read || shelvesResponse.read.length === 0) {
          setAllBooks([]);
          setInitialLoadDone(true);
          return;
        }

        const formatted: BookDisplay[] = shelvesResponse.read.map((item: ApiBook) => ({
        ...
        <Box style={styles.completedWrapper}>
          {loading && !initialLoadDone ? (
            <Center style={{ width: "100%", padding: 50 }}>
              <Text style={styles.centerText}>Loading completed books...</Text>
            </Center>
          ) : allBooks.length === 0 ? (
            <Center style={{ width: "100%", padding: 50 }}>
              <Text style={styles.centerText}>No completed books found.</Text>
            </Center>
          ) : (
